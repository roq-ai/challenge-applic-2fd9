...
import { getChallenges, deleteChallengeById } from 'apiSdk/challenges';
import { ChallengeInterface } from 'interfaces/challenge';

type ColumnType = ColumnDef<ChallengeInterface, unknown>;
...
export function ChallengeListPage(props: ChallengeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ChallengeInterface>>(
    () => `/challenges?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ChallengeInterface) => {
    if (hasAccess('challenge', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/challenges/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/challenges/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('challenge', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/challenges/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('challenge', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="challenge" />
        </Flex>

        {hasAccess('challenge', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/challenges/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
