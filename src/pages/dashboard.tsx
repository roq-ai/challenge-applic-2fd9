...
import { getChallengeResponses } from 'apiSdk/challenge-responses';
import { ChallengeResponseInterface } from 'interfaces/challenge-response';
import { getChallenges } from 'apiSdk/challenges';
import { ChallengeInterface } from 'interfaces/challenge';

import { ChallengeResponseListPage as ShowCaseList } from 'pages/challenge-responses';

function DashboardPage() {
  const { session } = useSession();

  const challengeResponseStat = useSWR<PaginatedInterface<ChallengeResponseInterface>>(
    () => `/challenge-responses`,
    () => getChallengeResponses({ limit: 1, offset: 0 }),
  );

  const challengeStat = useSWR<PaginatedInterface<ChallengeInterface>>(
    () => `/challenges`,
    () => getChallenges({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Challenge Responses',
      result: challengeResponseStat?.data?.totalCount,
      isLoading: challengeResponseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Challenges',
      result: challengeStat?.data?.totalCount,
      isLoading: challengeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Challenge Responses
                </Text>
                <NextLink href={`challenge-responses`} passHref legacyBehavior>
                  <Button
